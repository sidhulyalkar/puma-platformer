...
`JourneySave.Practiced` adds nine stable bits to the v1 schema without changing pickup, checkpoint, discovery, or waystone IDs. Existing v1 saves default to no practiced actions; negative values reset, unknown bits are masked, and unsupported versions reset with the rest of the save. Practice persists across deaths, travel, and trials; the short confirmation notice is transient. Actions may be practiced anywhere they are available, and practice never changes gameplay permissions or rewards.

Selected `Sign.Skills` masks choose the first unpracticed local lesson. Multi-skill signs accept either order and retire when all their skills have been observed. Untagged strategy signs remain. Signs require proximity and terrain line of sight; introductory suggestions stay hidden in trials, pause, recovery, and low passages. The HUD gives a crawl-clear reminder when a roll has ended under a ceiling, and exposes all nine notes in the field guide. `PracticeGuide.ObjectivePosition` is shared by the map and HUD: the outside arch, or the next trial mechanism.
