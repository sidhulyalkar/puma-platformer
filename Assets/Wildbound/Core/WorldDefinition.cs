...
{
    public enum Surface { Stone, Moss, Spring, Moving, Moonbridge, Balance, RootGate, Trailbridge, Vine, Bark, Ice }
    ...
    public sealed class Platform
    {
        ...
        public int MaxLandings, Landings;
        public float RespawnSeconds, BrokenTimer;
        public bool JustCracked, JustBroke;
        public float CrackProgress
        {
            get
            {
                ...
                if (!Enabled) return 1;
                return Landings / (float)MaxLandings;
            }
        }
        ...
        public Platform Fragile(int maxLandings, float respawnSeconds = 4.5f)
        { MaxLandings = maxLandings; RespawnSeconds = respawnSeconds; return this; }
        public void Update(float time)
        {
            Box next = Home.Offset(new V2((float)Math.Sin(time * .85f + Phase) * Travel, 0));
            Delta = next.Center - Bounds.Center; Bounds = next;
        }
        public void TickBroken(float dt)
        {
            if (MaxLandings <= 0 || Enabled) return;
            BrokenTimer -= dt;
            if (BrokenTimer <= 0) { Enabled = true; Landings = 0; BrokenTimer = 0; }
        }
        public void ClearFeedbackPulses() { JustCracked = false; JustBroke = false; }
        public void RegisterLanding()
        {
            if (MaxLandings <= 0 || !Enabled) return;
            Landings++;
            JustCracked = true;
            if (Landings >= MaxLandings)
            {
                Enabled = false;
                BrokenTimer = RespawnSeconds > 0 ? RespawnSeconds : 4.5f;
                JustBroke = true;
            }
        }
    }
    ...
    public sealed partial class WorldDefinition
    {
        ...
        public Box MapBounds = new Box(-8, -4, 96, 40);
        public V2 Start = new V2(2, 1);
        ...
        {
            if (biome < 0 || biome > 4) throw new ArgumentOutOfRangeException("biome");
            var w = new WorldDefinition { Biome = biome };
            w.Name = new[] { "THE AMBER CANOPY", "THE LANTERN GROTTO", "THE SKY GARDEN", "THE CINDER RAVINE", "THE FROSTGLASS RIDGE" }[biome];
            w.Subtitle = new[] { "Quiet paws beneath an amber moon.", "Wake the flowers. Follow their light.", "Hunt among the stars.", "Warm stone. Quiet fire. Climb the living bark.", "Ice that answers every step." }[biome];
            w.Memory = new[] { "The forest remembers every small beginning.", "Even the quietest places are full of life.", "Home is a trail you can choose again.", "Warmth is a trail you carry forward.", "Cold clarifies what the night already knew." }[biome];
            ...
            else if (biome == 3) BuildCinder(w);
            else BuildFrostglass(w);
            ...
            w.Signs.Add(new Sign(74, 1, "A DOOR TO SOMEWHERE", biome >= 4 ? "E / Y at the arch finishes the journey. Your wild places and waystones remain yours." : "E / Y at the arch discovers the next world. You can return through the map."));
            return w;
        }
    }
}
